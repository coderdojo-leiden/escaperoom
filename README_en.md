...
This is what it will look like when you have completed the command:
...
We start to put all sprites in a good starting position when we start our game. If something accidentally shifts during the game, we know that next time everything will be in the right place as we intended.
...
5. Secure it to the block above it
...
If all goes well, the house will now hide.

> Can you now do this for the **ontsnapt!2**-sprite?

### Show the other objects, with the good costume
...
Does your code work? To test!

> Can you now do this for the **schilderij**-sprite, the **kluis**-sprite, and the **deur**-sprite?

### Starting position

We also want every sprite to be shown in the right place on the screen, so that it looks the same every time. To do this, we let Scratch know where the image should be in relation to the center of the screen. From left to right is indicated with an 'x', and from bottom to top with a 'y'. For this, let's start with the carpet.
...
2. Now find the following block at *Motion*, and drag it to your code field just **above** the "show" block:  
   ...
   This way we set the place right before showing it.
3. Make sure the number after the `x` and the `y` is both a `0`

> Can you now also do this for the **schilderij**-sprite, the **kluis**-sprite, the **deur**-sprite, and the cottage (i.e. the **ontsnapt!**-sprite - with this right **above** the "hide"-block)?

## Look around and search the room

In an escape room you have to look for clues to escape. In our small room we want to build in the following things:
...
### Showing the vault code: Editing a look
...
7. Now click somewhere next to the painting on an empty spot, and type the first number of your own invented code. You will see these appear immediately. You may need to adjust the location and size a little more so that it is completely to your liking.
8. Do the same for the other numbers in your code, and put them in a slightly different place.
9. When you're done, click alternately on costume 1 and latest 2. Are you satisfied with the result? Otherwise, you may want to edit some more until you're happy

## Open the safe

If you want to open the safe, we have to ask for a code. And if that code has been entered correctly, the safe will open. What should we do for that?

#### Ask for the safe code: sensing

In Scratch, if you want to ask the user something, or see if maybe a key has been pressed, or if the mouse might be moving, it's called "Observing". We're going to use this here.
...
To do this, we use the blocks under "Control" in Scratch. This may be that we want to wait for something, or use an if-then question, or repeat something ourselves several times (or forever).

1. Now find the following block under *Control*, drag it to your code field, and lock it under the question `What is the code?`:  
   ![Control - if then](images/en/control-ifthen.png)
2. The `IF` and `THEN` we have. Now check if the answer is the same as our code on the wall.  
   Search for the next block at *Operators*, and drag it exactly in the right place on the if-then block:  
   ![Operators - equal to](images/operators-equal.png)
3. The answer to the question will appear on the left in this green block. Look this up at *Sensing* and drag it to the **left** box of the green block:  
...
5. An open vault is stored with the sprite as the second costume. Can you now change the costume within the if-then block if the code is correct? What do you need for this?

The code you entered after the if question is called a **condition**. Something that is true or false. Or which you can answer with Yes or No. In Scratch you can recognize these by the angular sides on a block:  
...
If you find the costume numbers difficult (what is 1 and 2 again?), you can also give each costume a *name*. This will make your code more readable:
    ...
    IF the costume is a closed vault
    THEN I have to ask for the code and check

> Can you see if you can change the name of the costumes to, for example, 'closed safe' and 'open safe'?  
> 
> What do you see change in the code block when the green flag is clicked?  
> 
> What could you change in your code block when the sprite is clicked to use this name?  
...
The key and the safe are two different sprites. Since you can only create Scratch blocks for one sprite, we need a way for one sprite to "talk" to another sprite. We do this with signals: The vault sends a message that it is open, and then any other sprite can listen to this message.
...
A signal is now set as soon as a correct code has been entered. Now we need to make the key respond to that message, and make it show.
...
The key may not show or may even be in the wrong place. We want the key to be "in" the vault: so the sprite must be shown in the foreground, and the position must also be correct. You've done this before:

1. First drag the key to the right place (i.e. in the safe) in the example screen at the top right
...
You should be able to test both blocks by double-clicking on them: With one it hides, and with the other it shows in front of the safe.

### Drag the key to the door: repeat

The intention is now that the key is dragged to the door in order to open the door.
...
In Scratch we can use a repetition. In such a block you tell *what* should be repeated all the time, and *when* it should stop. As with the if-then block, we need a **condition** to tell what happens to the angular blocks:  
![Condition](images/condition.png)
...
> Can you now complete the repeat block by using the blocks above in the right way?
...
The next step is to open the door if the key is used for the door. Then we hide the key (you don't need it anymore anyway). But if the key is released somewhere else (for example on the carpet), we want to put it back in the safe. That way the player knows that something else needs to be done with the key.

We can also say this in another 'code' way:
...
1. Now find the if-then-else block under *Control*, drag it to the code field, and lock it under the repeat block:  
   ![Control - if then else](images/en/control-ifthenelse.png)
2. At *Sensing*, find the condition if you hit something, and put it in the right place in the code block  

> To open the door, we have to use a signal again, for example a message `Door open`. Can you now set this signal yourself at the key?  

> Can you also open the door as soon as this signal is received?  
> Hint: you need to add this code to the **deur**-sprite!  

> Can you also hide the key after the door has been opened?  

> Can you return the key to the safe if it *didn't* hit the door?

## Escape

The last thing we need to do is go out the door and tell them you escaped!

### To go outside

> Can you add to the **deur**-sprite the event when the sprite is clicked, then we show the **ontsnapt!**-sprite (the cottage)?  
> Hint 1: You may only show the ontsnapt-sprite when the door is open! Use an if-then block to check if the door is really open (what look should the door have?)  
> Hint 2: Use a signal to tell the **ontsnapt!**-sprite to show. For example a message `Won`  

> Can you also make the **ontsnapt!2**-sprite (the message "you escaped!") appear when the player has escaped?  
